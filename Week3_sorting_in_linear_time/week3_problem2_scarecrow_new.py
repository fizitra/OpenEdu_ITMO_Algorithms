with open('scarecrow.in', 'r') as fin:
    ...
    '''
        ...
        0, k, 2*k , ... , n//k*k  в 1-й карман - эл-ты 1, k+1, ... , n//k*k + 1 и.т.д.
        Как только карман сформирован сортируем его
        Для того чтобы карманы были равной длины доплняем карман значением -1, если его длина
        меньше чем (n+k-1)//k
    '''
    for i in range(k):
        count = 0
        for j in range(i,(n+k-1)//k*k,k):
            ...
            count += 1
        buc_list[i].sort()
        buc_list[i] = buc_list[i] + [-1]*((n+k-1)//k-len(buc_list[i]))
